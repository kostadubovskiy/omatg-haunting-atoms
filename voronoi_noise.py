...
class VoronoiPhantomCellGenerator:
    ...
    def generate_phantoms(self, points: NDArray, atomic_numbers: NDArray, x_vec: NDArray, y_vec: NDArray, z_vec: NDArray) -> Generator[NDArray, None, None]:
        """
        ...
        while len(current_points) < self.desired_atom_count:
            next_point = self._get_next_point(current_points, current_atomic_numbers, x_vec, y_vec, z_vec)
            current_points = np.vstack([current_points, next_point])
            current_atomic_numbers = np.append(current_atomic_numbers, -1)
            yield next_point

    def _create_supercell(self, points: NDArray, atomic_numbers: NDArray, x_vec: NDArray, y_vec: NDArray, z_vec: NDArray) -> tuple[NDArray, NDArray]:
        """
        ...
        return np.concatenate(supercell_points), supercell_atomic_numbers

    def _get_farthest_point(self, vertices: NDArray, points: NDArray, atomic_numbers: NDArray) -> NDArray:
        """
        ...
        for vertice in vertices:
            ...
            weighted_distances = distances * np.log(1/(abs(atomic_numbers) + 1))
    ...
    def _get_next_point(self, points: NDArray, atomic_numbers: NDArray, x_vec: NDArray, y_vec: NDArray, z_vec: NDArray) -> NDArray:
        """
        ...
        supercell_points, supercell_atomic_numbers = self._create_supercell(centered_points, atomic_numbers, x_vec, y_vec, z_vec)
        
        voronoi_vertices = self._get_voronoi_vertices(supercell_points, x_vec, y_vec, z_vec)
        
        next_point = self._get_farthest_point(voronoi_vertices, supercell_points, supercell_atomic_numbers) + center_vector
        return next_point
