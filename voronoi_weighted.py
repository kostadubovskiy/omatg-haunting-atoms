...
class VoronoiPhantomCellGenerator:
    ...
    def _create_supercell_from_points(self, points: NDArray, atomic_numbers: NDArray, x_vec: NDArray, y_vec: NDArray, z_vec: NDArray) -> NDArray:
        """
        ...
        if len(supercell_points) != expected_points:
            raise ValueError(f"Supercell creation failed: expected {expected_points} points, but got {len(supercell_points)}.")
    ...
    def _get_farthest_point(self, vertices: NDArray, points: NDArray, atomic_numbers: NDArray) -> NDArray:
        ...
        for vertice in vertices:
            ...
            weighted_distances = distances * 1/(abs(atomic_numbers) + 1e5)
    ...
    def _get_next_point(self, points: NDArray, atomic_numbers: NDArray, x_vec: NDArray, y_vec: NDArray, z_vec: NDArray) -> NDArray:
        ...
        supercell_points = self._create_supercell_from_points(centered_points, atomic_numbers, x_vec, y_vec, z_vec)
        
        voronoi_vertices = self._get_voronoi_vertices(supercell_points, x_vec, y_vec, z_vec)
        
        next_point = self._get_farthest_point(voronoi_vertices, centered_points, atomic_numbers) + center_vector
        return next_point
